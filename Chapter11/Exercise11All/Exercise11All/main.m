...
void exercise4()
{
    ...
    do
    {
        printf("input the operate:");
        scanf("%s",str);
        [operate setString:[NSString stringWithUTF8String:str]];
        if([operate isEqualToString:@"sin"])
        {
            [calculator sin];
        }
        else if([operate isEqualToString:@"tan"])
        {
            [calculator tan];
        }
    ...
    while (![operate isEqualToString:@"exit"]);
    [operate release];
    [calculator  release];
}
void exercise5()
